...
- git 上的專案設定（`.csproj` 的 TargetFramework、`global.json`、套件版本）是**唯一基準**。任何機器都必須對齊專案，**不可反向修改專案**。除非遇到專案工具必須，也務必確認後才能更動版本。
- 遇到編譯失敗時，**嚴禁透過「改動專案的目標框架或工具版本」來讓它編過**。正確做法是：
  1. 先指出 git 基準要求的版本
  2. 比對本機目前安裝的版本
  3. 提示我在本機安裝/切換到符合專案的版本（該升升、該降降）
  4. 在我確認前，**不得修改任何專案內的版本設定檔**
- 與編譯/建置無關的工具版本差異，一律不處理。
