...
1. **搜尋連發多次 PING（提高探測可靠度）— 已提議，待實作**
   現況：搜尋只發 1 次 PING，單一回覆封包偶爾仍在半雙工碰撞中遺失 → 「有時找得到有時找不到」。
   提議：`ChatViewModel.PingAsync()` 在 5 秒掃描窗內**連發 3 次 PING（每次間隔 ~1.5s）**，
   BF8C 有多次機會回覆（各帶隨機退避），收到任一次即可。重複回覆依 DeviceId 去重，清單只顯示一台。
   → **這是「搜尋不到」的正解，名稱無關**（見上方第 7 點的結論，別再往名稱方向查）。
