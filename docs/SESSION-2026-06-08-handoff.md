...
### 後期（backlog，未做）
- ~~**定期廣播自身 GPS 位置**~~ → **韌體側已完成（FW 0.7.3，F-074）**：TYPE=0x07、HOP=1、間隔 NVS 可設（`pos_interval`，預設 30s）。**剩 APP 端解析 0x07 顯示於地圖/清單**（待做）。
...
### 📡 LoRa 距離設定：SF7/BW500 → SF9/BW125（FW 0.7.0）
- **症狀**：一台移到客廳、其他在房間，**不到 30m 穿牆就探測不到/收不到**。
- **根因**：原 SF7/BW500 是「最快但最短距、靈敏度最差」(~-123dBm)，當初為**語音速率**選的。
  發射功率已是最大 +22dBm，沒有再榨的空間 → 唯一大槓桿是 SF/BW。
- **改法**（`lora_handler.h`）：SF7→**SF9**、BW500→**BW125**，約 **+10dB 靈敏度**（距離數倍、穿牆大幅改善），
  代價速率 ~22kbps→~1.8kbps（每包 ~0.3-0.4s），對文字/SOS/探測足夠。**四台全燒 FW 0.7.0。**
- **⚠ 鐵則**：**所有節點 SF/BW 必須一致**，否則彼此完全收不到（實體層不同 channel）。改一台就要改全部。
- **語音待辦（Phase 4）**：Codec2 2400bps **塞不下 SF9/BW125(1.8kbps)**。屆時要嘛 PTT 時全網協調切到 SF7/BW500
  「短距高速模式」、要嘛改用 SF8/BW250(~6.25kbps，可跑語音且 +~6dB)。**做語音前要先決定這個。**
- **先排除天線**：C6L 有兩個 RP-SMA(WiFi/LoRa)，LoRa 天線鎖錯孔/沒鎖→距離直接崩，換設定前先確認。
