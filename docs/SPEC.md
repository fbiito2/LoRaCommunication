...
| **F-036** NodeDB 節點資料庫 | 參考 Meshtastic：從收到的**任何封包自動建檔**（ID/RSSI/最後聽到，最多 16 筆 LRU），PING 回覆學暱稱、POS/SOS 學座標。OLED 新增「Nodes」頁（總數 + 最近聽到前 4 台 ID/秒數）。✅ FW 0.7.5；**APP 端節點清單/地圖顯示待做** |
