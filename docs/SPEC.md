...
| **F-037** 節點地圖（規劃中）🟡 | APP 用 **MapLibre GL JS**（嵌 Blazor WebView，**一套跨 Android/iOS**，避開 Meshtastic 原生 MapLibre/MapKit 雙套分裂）顯示自己 + 各節點 markers（座標來自 F-074），點選顯示 ID/暱稱/RSSI/座標/最後聽到。底圖可切換 **線上 / 離線圖磚 / 雷達**。**階段一 ✅（待實機驗收）**：MapLibre GL JS（本地打包）+ **OpenFreeMap** 線上向量圖磚（免費免 key 可商用）；地圖頁「地圖」分頁、自己(青)/節點(黃) markers、點 marker→下方浮卡、☰ 開節點清單 dialog、🧭 回到自己。**階段二（待做）**：**離線打包/下載營運區域圖磚**——自算 OSM 向量 MBTiles/PMTiles 或 NLSC 官方圖資（合法、免費、高精度），符合「無基礎網路」災難場景；參考 Meshtastic 作法（MapLibre + OSM + 事先下載區域，離線無捷徑）。⚠ NLSC 打包散布前需再確認重製授權 |
